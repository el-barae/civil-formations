...
// Récupérer toutes les vidéos d'une formation avec les vues de l'utilisateur
exports.getVideosByFormation = async (req, res) => {
  try {
    ...
    const videos = await Video.findAll({
      ...
      order: [['numero', 'ASC']], // Trier par numéro
      include: [
        {
          model: View,
          where: { userId: userId },
          required: false, // LEFT JOIN pour inclure les vidéos sans vue
        }
      ]
    });
...
exports.updateVideo = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, numero, description } = req.body;
...
exports.deleteVideo = async (req, res) => {
  try {
    ...
    if (video.link) {
      const videoPath = path.join(__dirname, '..', video.link);
    ...
    await View.destroy({ where: { videoId: id } });
...
// Ajouter une vidéo à une formation (bonus)
exports.addVideoToFormation = async (req, res) => {
  try {
    const { id } = req.params; // ID de la formation
    const { title, numero, description } = req.body;
    ...
    const videoPath = `/uploads/privatevideos/${req.file.filename}`;
...
//       const videoPath = `uploads/privatevideos/${file.originalname}`;
...
//     }
//     const videosDir = path.join(__dirname, 'uploads/privatevideos');
//     if (!fs.existsSync(videosDir)) {
//         fs.mkdirSync(videosDir, { recursive: true });
//     }
//     const targetPath = path.join(videosDir, file[0].originalname);
//     fs.renameSync(file[0].path, targetPath);
//     const link = `uploads/privatevideos/${file[0].originalname}`;
